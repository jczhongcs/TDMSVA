...
MsHeader::MsHeader(xercesc::DOMElement* element) {
  ...
  xercesc::DOMElement* scan_element = xml_dom_util::getChildElement(element, "scan_list", 0);
...
double MsHeader::getPrecMonoMassMinusWater() {
  if (prec_mono_mz_ < 0 || std::isnan(prec_mono_mz_)) {
    ...
    return Peak::compPeakNeutralMass(prec_mono_mz_, prec_charge_) - mass_constant::getWaterMass();
  }
}
